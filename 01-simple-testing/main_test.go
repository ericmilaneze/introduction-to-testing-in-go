...
func Test_alpha_isPrime(t *testing.T) {
	...
	for _, e := range primeTests {
		...
		if e.expected && !result {
			t.Errorf("%s: expetected true but got false", e.name)
		}

		if !e.expected && result {
			t.Errorf("%s: expetected false but got true", e.name)
		}

		if e.msg != msg {
			t.Errorf("%s: exptected %s but got %s", e.name, e.msg, msg)
		}
	}
}
...
func Test_checkNumbers(t *testing.T) {
	tests := []struct {
		...
		{name: "zero", input: "1", expected: "1 is not prime, by definition!"},
	...
	for _, e := range tests {
		...
		if !strings.EqualFold(res, e.expected) {
			t.Errorf("%s : exptected %s, but got %s", e.name, e.expected, res)
		}
	}
}
